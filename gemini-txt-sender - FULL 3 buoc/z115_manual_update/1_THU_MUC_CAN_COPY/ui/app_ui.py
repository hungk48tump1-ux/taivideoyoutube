...
class App(tk.Tk):
    ...
    def __init__(self, config: Dict[str, Any], selectors: Dict[str, Any]):
        ...
        # ── BrowserThread (Playwright sống ở đây mãi mãi) ──
    ...
    def _left(self, p):
        ...
        def audio_panel(parent, title, audio_var, browse_cmd, model_var, engine_var, lang_var, beam_var, first4_var):
        ...
        audio_panel(audio_row, "🎙️ Trích Xuất Audio 1", self.v_audio, self._browse_audio, self.v_aud_model, self.v_aud_engine, self.v_aud_lang, self.v_aud_beam, self.v_aud_first4).grid(row=0, column=0, sticky="ew", padx=(0, 4))
        audio_panel(audio_row, "🎙️ Trích Xuất Audio 2", self.v_audio2, self._browse_audio2, self.v_aud_model2, self.v_aud_engine2, self.v_aud_lang2, self.v_aud_beam2, self.v_aud_first42).grid(row=0, column=1, sticky="ew", padx=(4, 0))
    ...
    def _top_controls(self, parent):
        ...
        for text, cmd, bg in [
            ...
            ("🚀 Mở Chrome", self._launch, C["accent2"]),
            ("🔄 Đổi Hồ Sơ", self._relaunch, "#7b3f00"),
            ("⏸ Tạm dừng", self._do_pause, C["warn"]),
            ("⏯ Tiếp tục", self._do_resume, C["info"]),
    ...
    def _add_url(self):
        ...
        def _save():
            ...
            self._saved_urls[name] = {"url": url, "ref_image": ref}
        ...
        bbar.grid(row=3, column=0, columnspan=2, pady=20)
        tk.Button(bbar, text="Lưu thông tin", bg=C["ok"], fg="white", font=FS, command=_save).pack()
    ...
    def _launch(self):
        ...
        self.config["gemini_url"] = url
        self.log(f"Đang khởi động browser → {url}")
        self.v_status.set("Đang mở browser...")

        def task():
            try:
                profile_name = getattr(self, "v_internal_profile", tk.StringVar(value="Profile 1")).get()
                folder_name = f"browser_{profile_name.replace(' ', '_').lower()}"
                self._bc("launch", headless=False, user_data_dir_name=folder_name, timeout=60)
                self._bc("navigate_to_gemini", timeout=40)
    ...
    def _relaunch(self):
        ...
        url = self._get_active_url() or "https://gemini.google.com/app"
        self.config["gemini_url"] = url
        self.log("🔄 Đang reset và khởi động lại browser...", "WARNING")
        self.v_status.set("Đang reset...")

        def task():
            try:
                self._bc("reset", timeout=15)
            except Exception:
                pass
            try:
                profile_name = getattr(self, "v_internal_profile", tk.StringVar(value="Profile 1")).get()
                folder_name = f"browser_{profile_name.replace(' ', '_').lower()}"
                self._bc("launch", headless=False, force_close=True, user_data_dir_name=folder_name, timeout=90)
                self._bc("navigate_to_gemini", timeout=40)
                self.log("✅ Đã khởi động lại thành công!", "SUCCESS")
                self.v_status.set("Browser sẵn sàng")
            except Exception as e:
                self.log(f"❌ Lỗi khi relaunch: {e}", "ERROR")
                self.v_status.set("Lỗi")
        threading.Thread(target=task, daemon=True).start()
    ...
    def _start(self):
        ...
        has_file2 = bool(self.v_audio2.get().strip() or self.v_file2.get().strip())
        if has_file2:
        ...
        if audio_path:
            ...
            self._stop.clear()
            self._pause.set()
            self._worker = threading.Thread(target=self._run_audio_worker, args=(audio_path,), daemon=True, name="AudioWorker")
            self._worker.start()
        elif txt_path:
            if not do_gemini:
                if do_pipeline:
    ...
    def _start_consecutive(self):
        ...
        jobs = [
            {"name": "File 1", "audio": self.v_audio.get().strip(), "txt": self.v_file.get().strip(), "preset": self.v_url.get().strip()},
            {"name": "File 2", "audio": self.v_audio2.get().strip(), "txt": self.v_file2.get().strip(), "preset": self.v_url2.get().strip()},
        ]
        jobs = [job for job in jobs if job["audio"] or job["txt"]]
        if not jobs:
            messagebox.showwarning("Thiếu file", "Vui lòng chọn ít nhất 1 file Audio hoặc TXT để chạy.")
            return
    ...
    def _run_consecutive_worker(self, jobs: List[Dict[str, Any]]):
        ...
        if not (self.v_do_audio.get() and jobs[0].get("audio")):
                ...
                if self.v_do_audio.get():
                    ...
                    if job.get("audio"):
                        with locks["audio"]:
                            ...
                            self.log_job(job_name, "Bước 1: trích xuất audio.", "INFO")
    ...
    def _retry_chunk(self):
        self.log("🔁 Sẽ gửi lại chunk hiện tại trong lần chạy tiếp.", "WARNING")
    ...
    def _run_worker(self, file_path: str, auto_pipeline: bool = True, job_config: Optional[Dict[str, Any]] = None):
        ...
        try:
            import os
            import subprocess
            browser_path = self.v_custom_browser_path.get().strip()
            if browser_path and os.path.exists(browser_path):
                self.log(f"🌐 Đang mở liên kết bằng trình duyệt cấu hình: {browser_path}", "INFO")
                subprocess.Popen([browser_path, url_with_param])
            else:
                import webbrowser
                # Tự động tìm Chrome nếu đường dẫn trống nhưng có cài Chrome
                chrome_paths = [
                ...
                for cp in chrome_paths:
                    if os.path.exists(cp):
                        self.log(f"🌐 Tìm thấy Chrome tại đường dẫn mặc định: {cp}", "INFO")
                        subprocess.Popen([cp, url_with_param])
                        found_chrome = True
                        break
                
                if not found_chrome:
                    self.log(f"🌐 Đang mở liên kết tự động bằng trình duyệt mặc định: {url_with_param}", "INFO")
                    webbrowser.open(url_with_param)
        except Exception as e:
            self.log(f"⚠️ Không tự động mở được trình duyệt: {e}", "WARNING")
        ...
        for chunk in chunks:
            ...
            (self.v_cur2 if job_name == "File 2" else self.v_cur).set(
            ...
            while True:
                ...
                retries += 1
                self.log(f"Lần gửi {retries} cho chunk {chunk.index}...", "INFO" if retries == 1 else "WARNING")
                ...
                if success:
                    ...
                    self.log(f"⚠️ Chunk {chunk.index} lỗi. Extension đang tự động reload trang và gửi lại...", "WARNING")
                    time.sleep(3)
                    continue
                else:
                    # Lỗi thật sự → dừng
            ...
            if success:
                ...
                # ── Chờ Extension tự động reload (10s chờ đồng bộ + reload trang) ──
                if chunk.index < total:
                    self.log("⏳ Chờ 15 giây để Extension hoàn tất đồng bộ đám mây và tự động reload...", "INFO")
                    time.sleep(15)
            else:
                session.mark_chunk_failed(chunk.index, response_text[:200])
                self.log(f"❌ Chunk {chunk.index} thất bại: {response_text[:200]}", "ERROR")
                all_chunks_ok = False
                break
